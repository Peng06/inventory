...
var attachmentsInventory = document.getElementById("attachments-inventory");
...
window.addEventListener("drop", function(event) {
  event.preventDefault();
  var weapon = event.dataTransfer.getData("text");
  ...
  window.addEventListener("message", function (event) {
    if (event.data.action == "setWeaponAttachments") {
      ...
      attachmentsInventory.innerHTML = "";
      ...
      for (var i = 0; i < attachments.length; i++) {
        ...
        attachmentsInventory.appendChild(attachmentBox);
      }
    }
  });
});
...
function showHotbar(hotbarItems) {
    $("#hotbar").fadeIn(200);
    for(i = 1; i < 6; i++) {
        $("#hotbar").append('<div class="slot" data-hotbar="' + i + '"><div class="slot-number">' + i + '</div></div>');
    }
    ...
    $.each(hotbarItems, function (index, item) {
        ...
        $("#hotbar").find("[data-hotbar=" + item.slot + "]").html('<div class="slot-number">' + item.slot + '</div><div id="item-' + item.slot + '" class="item" style = "background-image: url(\'img/' + image + '.png\')">' +
...
function inventorySetup(items, hotbarItems, weight, maxWeight) {
    ...
    for (let i = 1; i < 6; i++) {
        $("#thirdLocation").append('<div class="hotbar-weapons" id="hotbar-' + i + '"><div class="slot-number">' + i + '</div><div data-hotbar="' + i +  '"></div></div>');
        ...
        $(id).droppable({
            ...
            drop: function (event, ui) {
                ...
                if (event.shiftKey && event.which === 1 && event.originalEvent.detail === 2) {
                    for (let j = 1; j < 6; j++) {
                        if ($(`#hotbar-${j}`).children().length === 1) {
    ...
    $.each(hotbarItems, function (index, item) {
        ...
        $("#thirdLocation").find("[data-hotbar=" + item.slot + "]").html('<div id="item-' + item.slot + '" class="item" style = "background-image: url(\'img/' + image + '.png\')">' +
...
$(document).ready(function () {
    ...
    $('#attachments').droppable({
        ...
        drop: function (event, ui) {
            ...
            disableInventory(300);
            $.post("http://vrp_inventoryhud/OpenAttachments", JSON.stringify({
                item: itemData
            }));
        }
    });
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "attachments" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://vrp_inventoryhud/TakeFromAttachments", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "attachments" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://vrp_inventoryhud/PutIntoAttachments", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));                
